...
username = "CheeseChessBot"

session_op = berserk.TokenSession(token_black)
client_op = berserk.Client(session=session_op)

def play_game():
    challenge = client.challenges.create(username="CheeseChessBot2", rated=False, color="white")
    c_id = challenge['id']
    client_op.challenges.accept(c_id)
    return c_id

def resign(c_id):
    client.bots.resign_game(c_id)

def move(c_id, place):
    client.bots.make_move(c_id, place)
    return "move ok"

def move_b(c_id, place):
    client_op.bots.make_move(c_id, place)
    return "b move ok"

def get_board(c_id):
    board_state = client.bots.stream_game_state(c_id)
    return board_state

def get_last_games(c_id):
    game = client.games.export(c_id)
    all_uci_moves = []
